...
cvd = cvd %>%
  # death case codings: 1 if died from other causes, 2 if died from the disease in question (based on cause_death), 0 if still alive
  mutate(time = ifelse(death_case==2 & date_death<=date_censor, date_death-date_recr, 
                       ifelse(death_case==2 & date_death>date_censor, date_censor-date_recr,
                              ifelse(death_case==1 & date_death<=date_censor, date_death-date_recr, 
                                     ifelse(death_case==1 & date_death>date_censor, date_censor-date_recr, 
                                            date_censor-date_recr))))) %>%
  
  mutate(status = ifelse(death_case==2 & date_death<=date_censor, 1, # died from the disease
                         ifelse(death_case==1, 0, # censor died from other causes
                                ifelse(death_case==0, 0, 0)))) # alive
#cvd$time = as.numeric(cvd$time)
#cvd$time = with(cvd, ifelse(is.na(time), date_censor-date_recr, time))


# Extract required columns                         
cvd_m = cvd[, c("eid" ,"status", "time")]

# Merge onto main dataset
bb = merge(bb, cvd_m, by.x="row.names", by.y="eid")
rownames(bb)=bb$Row.names
bb = within(bb, rm(Row.names))

# Change age to be on the same scale as time
...
annotate_figure(p, 
                top = text_grob("CVD Mortality", face = "bold"),
